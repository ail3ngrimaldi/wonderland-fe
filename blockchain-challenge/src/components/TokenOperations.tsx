import {
  ...
  import { SEPOLIA_CONTRACTS, ERC20_ABI, TOKEN_DECIMALS }
  ...
  | 'transfer'>('transfer')
    ...
    const { writeContract, isPending, data: hash, error,
  reset } = useWriteContract()
  ...
  !address) {
      return (
        <Alert severity="warning" sx={{ mb: 2 }}>
          ⚠️ Connect to Sepolia network to
  ...
  !isNaN(Number(amount)) && Number(amount) > 0
    const isValidRecipient = operation === 'transfer' ?
  isAddress(recipientAddress) : true
    const isValidSpender = operation === 'approve' ?
  isAddress(spenderAddress) : true
  ...
  setOperation(e.target.value as 'approve' | 'transfer')}
            >
              <MenuItem value="transfer">Transfer (send
  tokens)</MenuItem>
              <MenuItem value="approve">Approve (give
  permission)</MenuItem>
            </Select>
          </FormControl>
  ...
  setSelectedToken(e.target.value as TokenType)}
            >
              <MenuItem value="DAI">DAI (18
  decimals)</MenuItem>
              <MenuItem value="USDC">USDC (6
  decimals)</MenuItem>
            </Select>
          </FormControl>
          ...
          <TextField
            ...
            helperText={amount !== '' && !isValidAmount ?
  ...
  !isValidRecipient}
              helperText={recipientAddress !== '' &&
  !isValidRecipient ? 'Enter a valid Ethereum address' :
  ...
  setSpenderAddress(e.target.value)}
              error={spenderAddress !== '' &&
  !isValidSpender}
              helperText={spenderAddress !== '' &&
  !isValidSpender ? 'Enter a valid Ethereum address' :
  'Address that can spend your tokens'}
          ...
          <Button
            fullWidth
            variant="contained"
            onClick={operation === 'approve' ? 
  handleApprove : handleTransfer}
            disabled={!canSubmit || isPending || 
  isConfirming}
            startIcon={isPending || isConfirming ? 
  <CircularProgress size={16} /> : null}
          >
            {isPending ? `Signing ${operation}...` :
             isConfirming ? `Mining ${operation}...` :
             operation === 'approve' ? 'Approve Tokens' :
   'Transfer Tokens'}
          </Button>
