...
const mockFormatHash = vi.fn((hash: string) => `${hash.slice(0, 6)}...${hash.slice(-4)}`)
...
describe('EventTable', () => {
  ...
  describe('When there are no transactions', () => {
    it('should show empty state message', () => {
      ...
      expect(screen.getByText('Your Impact History (0 actions)')).toBeInTheDocument()
      expect(screen.getByText('No impact actions yet. Start by planting some seeds!')).toBeInTheDocument()
  ...
  describe('When there are transactions', () => {
    ...
    const mockTransactions = [MOCK_TRANSACTIONS.MINT_TX, MOCK_TRANSACTIONS.TRANSFER_TX]
    ...
    beforeEach(() => {
      ...
      mockUseTransactionDetails.mockImplementation((hash: string) => ({
        data: hash === MOCK_TRANSACTIONS.MINT_TX.hash ? {
    ...
    it('should display transaction count and clear button', () => {
      ...
      expect(screen.getByText('Your Impact History (2 actions)')).toBeInTheDocument()
