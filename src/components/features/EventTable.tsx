...
function TransactionRow({
  ...
  const { data: enrichedData, isLoading } = useTransactionDetails(
    transaction.hash
  )
  ...
  const getActionLabel = (type: string) => {
    switch (type) {
      case 'mint':
        return '🌱 Planted Seeds (MINT)'
      case 'transfer':
        return '💚 Made Impact (TRANSFER)'
      case 'approve':
        return '🤝 Sponsored Project (APPROVE)'
      default:
      ...
      <TableCell>
        {isLoading ? (
          ...
          <Typography variant="body2" sx={{ fontFamily: 'monospace' }}>
            Block: {enrichedData.blockNumber.toString()}
            <br />
